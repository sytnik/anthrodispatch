...
/// <summary>Baseline GA + AWM mutation only (no CPC).</summary>
public sealed class AwmGaService
{
        ...
        for (var gen = 0; gen < _options.MaxGenerations; gen++)
        {
            ...
            while (newPop.Count < _options.PopulationSize)
            {
                ...
                var child = _rng.NextDouble() < _options.CrossoverProbability
                    ? _core.TwoPointCrossover(p1, _core.TournamentSelect(population, _options.TournamentSize))
                    : p1.DeepClone();
